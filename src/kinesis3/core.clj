    ...
    (try
      (defroutes app-routes
        ...
        (GET "/stats" request {:status 200 :headers {"Content-Type" "application/json"}
                               :body (generate-string {:events (rates message-ingested)
                                                       ...
                                                       :s3-upload-timing {:percentile (timers/percentiles s3-upload-timing)
                                                                          ...
                                                                          :mean (/ (timers/mean s3-upload-timing) 100000)}
...
(defn http-server [port] (map->HTTP {:port port}))

(defn gen-temp-file [] (java.io.File/createTempFile "records" ".log.gz"))
